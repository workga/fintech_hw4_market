...
from app.market.database import clear_db, create_session, init_db
...
def mock_db_exception(mocker):
    mocker.patch('sqlalchemy.orm.Session.add', side_effect=SQLAlchemyError)
    mocker.patch('sqlalchemy.orm.Query.all', side_effect=SQLAlchemyError)
    mocker.patch('sqlalchemy.orm.Query.one', side_effect=SQLAlchemyError)
    mocker.patch('sqlalchemy.orm.Query.first', side_effect=SQLAlchemyError)
    mocker.patch('sqlalchemy.orm.Query.one_or_none', side_effect=SQLAlchemyError)


def formatted_now(time_shift=timedelta()):
    return datetime.strftime(
        datetime.now() + time_shift - timedelta(hours=3), DATETIME_FORMAT
    )
